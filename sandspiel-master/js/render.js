...
let startWebGL = ({ canvas, universe, isSnapshot = false }) => {
  ...
  // The wasm cell buffer is height-major (index = x * height + y), so the
  // texture's row length must be `height`. For a square grid width === height
  // (no-op); for a rectangular grid this keeps the cells spatially coherent.
